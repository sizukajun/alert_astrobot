...
def check_market():
    ...
    try:
        ...
        # 1. 공지사항을 제외한 모든 행을 가져옵니다.
        rows = soup.select('.kboard-list tbody tr:not(.kboard-list-notice)')
        print(f"🔎 매물 후보 {len(rows)}개 발견. 상위 5개를 정밀 검사합니다.")

        for row in rows[:5]:
            # 2. 클래스명 대신 '칸(td)'의 순서로 데이터를 뽑습니다 (가장 확실함)
            cols = row.find_all('td')
            if len(cols) < 2:
                continue

            # 보통 첫 번째 칸이 번호(ID), 두 번째 칸이 제목입니다.
            uid_el = row.select_one('.kboard-list-uid') or cols[0]
            title_cell = row.select_one('.kboard-list-title') or cols[1]
            
            # 제목 내부의 링크(a) 태그를 찾습니다.
            ...
            link = "https://astromart.co.kr" + link_el['href']
            
            print(f"📝 검사 중: [{post_id}] {title}")
